...
impl AtomicMeters {
    ...
    pub fn set_channels(&self, channels: usize) {
        self.live_channels.store(channels as u32, Ordering::Relaxed);
    }

    /// The live channel count, or `None` if no backend has reported one.
    pub fn channels(&self) -> Option<usize> {
        match self.live_channels.load(Ordering::Relaxed) {
            0 => None,
            n => Some(n as usize),
        }
    }
